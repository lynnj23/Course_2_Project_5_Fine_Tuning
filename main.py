...
from transformers import AutoTokenizer as att
...
# Stream or slice later dependent on the accuracy of th training.
...
def preprocess_function(examples):
    ...
    for txt, s_label in zip(examples["text"], examples["split"]):
        if s_label == "test":
            ...
            # Tokenising non-test data here. This is a bit ugly, but I have got myself in a twist here at moment
            tokenized = tokenizer(txt, truncation=True, padding="max_length")
            token_dataset.append(tokenized["input_ids"])
...
# Output the text classification results in table format comparing before and after outputs.
